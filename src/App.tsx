...
import { BuildingScalableLaravel } from "./pages/blog/BuildingScalableLaravel";
import { ReactPerformanceOptimization } from "./pages/blog/ReactPerformanceOptimization";
import { FlutterStateManagement } from "./pages/blog/FlutterStateManagement";
...
export function App() {
  ...
  return (
    <>
      <Routes>
        <Route path="/blog/building-scalable-laravel-applications" element={<BuildingScalableLaravel />} />
        <Route path="/blog/react-performance-optimization" element={<ReactPerformanceOptimization />} />
        <Route path="/blog/flutter-state-management-guide" element={<FlutterStateManagement />} />
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      parallaxBgs.forEach((bg, index) => {
        ...
        element.style.transform = `translate3d(${-x * 100 * parallaxFactor}px, ${-y * 100 * parallaxFactor}px, 0) scale(${1 + Math.abs(x * y) * 0.1})`;
      });
    };
    const handleScroll = () => {
      ...
      document.querySelectorAll(".perspective-card, .perspective-element").forEach((el) => {
        ...
        if (rect.top < viewportHeight && rect.bottom > 0) {
          ...
          const r = Math.min(Math.max(scrollProgress, 0), 1);
          element.style.transform = `perspective(1000px) rotateX(${r * 5}deg) translateZ(${r * 15}px) scale(${1 + r * 0.05})`;
        }
      });
    };
